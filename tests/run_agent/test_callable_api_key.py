...
class TestInlinedDisplayMasks:
    ...
    def test_anthropic_401_diagnostic_handles_callable(self):
        """``run_agent`` Anthropic 401 diagnostic path used to do
        ``key[:12]`` on ``self._anthropic_api_key``. For Entra ID +
        ...
        assert "Microsoft Entra ID (httpx event hook)" in src, (
            "run_agent Anthropic 401 diagnostic must surface a "
            "Microsoft Entra ID branch before slicing the key prefix."
        )
