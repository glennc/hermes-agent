...
| `AZURE_FOUNDRY_API_KEY` | Azure AI Foundry / Azure OpenAI API key ([ai.azure.com](https://ai.azure.com/)). Not needed when `model.auth_mode: entra_id` |
...
| `AZURE_CLIENT_ID` | Entra ID client ID (service principal or user-assigned managed identity) |
...
| `IDENTITY_ENDPOINT` / `MSI_ENDPOINT` | Managed Identity endpoint; set automatically by Azure-hosted compute (VMs, Functions, App Service, Container Apps) |
